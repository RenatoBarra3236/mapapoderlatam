...
function Chatbot({ caseData, lang }) {
  ...
  // Simulate AI response — pulls from real case data
  const fakeAnswer = (q) => {
    ...
    <div className={`chatbot ${expanded ? "expanded" : "collapsed"}`}>
      <div className="chatbot-head" onClick={() => setExpanded(e => !e)}>
        <div className="chatbot-head-l">
          ...
          <div className="chatbot-title">{t.askProfile} <em>· {caseData.nodes.find(n=>n.id===caseData.rootId).name.split(" ").slice(0,2).join(" ")}</em></div>
        </div>
