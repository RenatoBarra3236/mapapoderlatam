...
import { chatWithProfile } from '../../lib/api';
...
export default function ChatbotDrawer({ caseData, lang }) {
  ...
  const send = async (text) => {
    ...
    try {
      const result = await chatWithProfile(v, caseData, lang, historyForApi);
  ...
  return (
    <div className={`chatbot ${expanded ? 'expanded' : 'collapsed'}`}>
      <button
        ...
        aria-label={expanded
          ? (lang === 'es' ? `Cerrar asistente · ${shortName}` : `Close assistant · ${shortName}`)
          : (lang === 'es' ? `Abrir asistente · ${shortName}` : `Open assistant · ${shortName}`)
        }
      >
      ...
      {expanded && (
        <div className="chatbot-body" id="chatbot-body">
          <div className="chat-messages" ref={msgsRef}>
            {messages.length === 0 && (
              <div style={{ color: 'var(--ink-3)', fontSize: 13, fontStyle: 'italic', fontFamily: 'var(--font-serif)' }}>
                {lang === 'es'
                  ? 'Haz una pregunta sobre este perfil — por ahora responde con reglas sobre la red visible.'
                  : 'Ask a question about this profile — for now it answers with rules over the visible network.'}
              </div>
            )}
            {messages.map((m, i) => (
          ...
          <div className="suggested-row">
            {sugg.map((q, i) => (
              <button type="button" key={i} className="suggested-chip" onClick={() => send(q)}>{q}</button>
            ))}
          </div>
